...
export class DattoHttpClient {
  ...
  private async fetchToken(): Promise<TokenEntry> {
    ...
    if (res.status === 401) {
      throw Object.assign(new Error('DattoRMM: invalid API credentials'), { failParent: true });
    }
  ...
  async get<T>(url: string): Promise<T> {
    ...
    if (res.status === 401) {
      throw Object.assign(new Error('DattoRMM auth rejected'), { failParent: true });
    }
    if (!res.ok) throw new Error(`DattoRMM API error ${res.status}: ${fullUrl}`);
  ...
  async post<T>(url: string, body: unknown): Promise<T> {
    ...
    if (!res.ok) throw new Error(`DattoRMM POST error ${res.status}: ${fullUrl}`);
    return res.json() as Promise<T>;
  }
  ...
  async put<T>(url: string, body: unknown): Promise<T> {
    ...
    if (!res.ok) throw new Error(`DattoRMM PUT error ${res.status}: ${fullUrl}`);
    return res.json() as Promise<T>;
  }
}
