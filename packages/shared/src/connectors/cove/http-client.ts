...
export class CoveHttpClient {
  ...
  private async fetchVisa(): Promise<string> {
    ...
    if (!res.ok) {
      throw Object.assign(
        new Error(`Cove login HTTP error: ${res.status}`),
        { failParent: true }
      );
    }
    ...
    if (data.error) {
      throw Object.assign(
        new Error(`Cove login error: ${data.error.message} (code ${data.error.code})`),
        { failParent: true }
      );
    }
  ...
  async rpc<T>(method: string, params: unknown): Promise<T> {
    ...
    if (body.error) {
      ...
      throw new Error(`Cove RPC error: ${body.error.message} (code ${body.error.code})`);
    }
