...
import { syncRunStages, syncContext, entityChangeLog, rawIngestLog } from './index.js';
import type { MspServiceDb } from '../../clients.js';
...
export async function startStage(
  ...
  const [row] = await db
    ...
    .values({ syncRunId, integrationId, bullmqJobId, type, stage, status: 'running', startedAt: new Date() })
    .returning();
  return row!.id;
}
...
export async function completeStage(
  ...
  await db.update(syncRunStages).set({
    status: 'completed',
    finishedAt: new Date(),
...
export async function failStage(db: MspServiceDb, stageId: string, error: unknown): Promise<void> {
...
export async function recordFetchSuccess(
  ...
  const now = new Date();
  await db.insert(syncContext).values({
    linkId,
    integrationId,
    type: facet,
...
export async function recordFetchFailure(
  ...
  const msg = error instanceof Error ? error.message : String(error);
  const kind = (error as { kind?: string })?.kind ?? (error instanceof Error ? error.name : 'Error');
  const now = new Date();
  ...
  await db.insert(syncContext).values({
    ...
    target: [syncContext.linkId, syncContext.integrationId, syncContext.type],
    set: { consecutiveFailures: newCount, lastFailureAt: now, lastErrorClass: kind, lastErrorMessage: msg, updatedAt: now },
  });
}
...
export async function logRawRecords(
  ...
  const rows = records.map((payload, i) => ({
    ...
    syncRunId,
    type: facet,
    externalId: String((payload as Record<string, unknown>)?.id ?? (payload as Record<string, unknown>)?.externalId ?? i),
