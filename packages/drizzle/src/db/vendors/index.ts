...
export const sophosEndpoints = vendorsSchema.table(
  'sophos_endpoints',
  {
    ...
    previousCodes: text('previous_codes').array().notNull().default([]),
...
export const dattoEndpoints = vendorsSchema.table(
  'datto_endpoints',
  {
    ...
    lastRebootAt: timestamp('last_reboot_at', { withTimezone: true }).notNull(),
    lastHeartbeatAt: timestamp('last_heartbeat_at', { withTimezone: true }),
    lastSeenAt: timestamp('last_seen_at', { withTimezone: true }).notNull().defaultNow(),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
...
export const m365AuthMethods = vendorsSchema.table(
  'm365_auth_methods',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    linkId: uuid('link_id').notNull().references(() => integrationLinks.id),
    ...
    type: text('type').notNull(),
    creationDateAt: timestamp('creation_date_at', { withTimezone: true }),
    meta: jsonb('meta'),
    lastSeenAt: timestamp('last_seen_at', { withTimezone: true }).notNull().defaultNow(),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
...
export const m365Devices = vendorsSchema.table(
  'm365_devices',
  {
    ...
    approximateLastSignInAt: timestamp('approximate_last_sign_in_at', { withTimezone: true }),
