...
export const fieldCompareEvaluator: CheckTypeEvaluator = {
  async evaluate(config, ctx: EvalContext): Promise<EvalResult> {
    ...
    const { table, filter, field, op = 'eq', value } = parsed.data;
    ...
    if (!row) return { passed: false, detail: { reason: 'no matching row found' } };

    const actual = getNestedValue(row, field);
    return { passed: evalFieldOp(actual, op, value), detail: { field, op, expected: value, actual } };
  },
};
