...
async function syncOrgWorkers() {
  ...
  for (const orgId of activeOrgIds) {
    ...
    const worker = createComplianceWorker(redis, orgQueueName(QUEUES.COMPLIANCE, orgId));
    workersByOrg.set(orgId, worker);
    logger.info({ orgId }, 'Started compliance worker for org');
  }
  ...
  for (const [orgId, worker] of workersByOrg) {
    ...
    await worker.close();
    logger.info({ orgId }, 'Closed compliance worker for org');
  }
}
...
const orgWorkerRefresh = setInterval(() => {
  void syncOrgWorkers().catch((err) => logger.error({ err }, 'Failed to sync compliance workers'));
}, 60_000);
...
logger.info({ orgs: workersByOrg.size }, 'Compliance org workers started');
