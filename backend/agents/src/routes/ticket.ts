  ...
  fastify.post('/api/v1.0/ticket/create', async (req, reply) => {
    ...
    if (psaSiteId) {
      try {
        ...
        if (match) {
          assetIds = [match.id];
          logger.info({ assetId: match.id, hostname: agent.hostname }, 'HaloPSA asset matched');
        }
      } catch {
        logger.warn({ siteId, hostname: agent.hostname }, 'Failed to fetch HaloPSA assets');
      }
    }
    ...
    if (body.email) {
      try {
        ...
        logger.info({ contactId, email: body.email }, 'HaloPSA contact found');
      } catch {
    ...
    if (body.screenshot?.data) {
      try {
        ...
        logger.info({ hostname: agent.hostname }, 'Screenshot uploaded to HaloPSA');
      } catch {
        logger.warn({ hostname: agent.hostname }, 'Failed to upload screenshot to HaloPSA');
      }
    }
