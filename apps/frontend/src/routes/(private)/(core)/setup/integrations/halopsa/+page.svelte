<script lang="ts">
  ...
  const integrationQuery = createQuery(() => ({
    queryKey: ['integrations.get', 'halopsa'],
    queryFn: () => trpc.integrations.get.query({ id: 'halopsa' }),
  }));
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        This will remove the HaloPSA integration and all associated site mappings. This action can
