<script lang="ts">
  ...
  const integrationQuery = createQuery(() => ({
    queryKey: ['integrations.get', 'dattormm'],
    queryFn: () => trpc.integrations.get.query({ id: 'dattormm' }),
  }));
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        This will remove the DattoRMM integration and all associated site mappings. This action can
        be undone within 30 days.
      </AlertDialog.Description>
    </AlertDialog.Header>
    <AlertDialog.Footer>
  ...
  {:else}
    <div class="flex flex-col size-full justify-center items-center">
      <div class="flex items-center gap-3 px-4 py-3 w-fit rounded bg-warning/10 text-warning border border-warning/30">
        ...
        <span class="text-sm">
          DattoRMM is not configured yet. Click <strong>Configure</strong> to set up your
          credentials.
        </span>
      </div>
    </div>
