<script lang="ts">
  ...
  const integrationQuery = createQuery(() => ({
    queryKey: ['integrations.get', 'cove'],
    queryFn: () => trpc.integrations.get.query({ id: 'cove' }),
  }));
  ...
  $effect(() => {
    data.customers
      .then((customers: CoveChildPartner[]) => {
        externalOptions = customers.map((c) => ({
          id: String(c.Info.Id),
          name: c.Info.Name,
        }));
      })
...
<AlertDialog.Root bind:open={showDeleteConfirm}>
  <AlertDialog.Content>
    <AlertDialog.Header>
      <AlertDialog.Title>Delete Cove Backups Integration?</AlertDialog.Title>
      <AlertDialog.Description>
        This will remove the Cove Backups integration and all associated site mappings. This action
