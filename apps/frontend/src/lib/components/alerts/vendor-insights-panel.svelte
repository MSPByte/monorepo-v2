<script lang="ts">
  ...
  let {
    ...
    moduleLabelForDefinition = (definitionId: string) => definitionId,
    onalertchange,
  }: {
    ...
    moduleLabelForDefinition?: (definitionId: string) => string;
    onalertchange?: () => void;
  } = $props();
  ...
  function selectedFilter() {
    return filterOptions.find((filter) => filter.id === activeFilter);
  }

  const scopeEnabled = $derived(!!siteId || !!linkId);
  ...
  const groupsQuery = createQuery(() => ({
    ...
    queryFn: () => {
      ...
      return trpc.alerts.insightGroups.query({
        siteId: siteId ?? undefined,
        linkId: siteId ? undefined : (linkId ?? undefined),
        integrationId,
        status: 'active',
        definitionPrefixes: filter?.definitionPrefixes,
        definitionExcludePrefixes: filter?.definitionExcludePrefixes,
  ...
  const countsQuery = createQuery(() => ({
    queryKey: ['alerts.insightGroupCounts', integrationId, siteId, linkId, 'active', searchQuery.trim()],
    queryFn: () =>
      trpc.alerts.insightGroupCounts.query({
        ...
        status: 'active',
        search: searchQuery.trim() || undefined,
        buckets: filterOptions.map((filter) => ({
          id: filter.id,
          definitionPrefixes: filter.definitionPrefixes,
          definitionExcludePrefixes: filter.definitionExcludePrefixes,
        })),
...
<div class="flex flex-col size-full overflow-hidden">
  <div class="flex items-center justify-between px-4 py-2.5 border-b shrink-0">
    ...
    {#if !loading}
      <a href={alertsHref} class="text-xs text-muted-foreground hover:text-foreground transition-colors">
      ...
      <table class="w-full table-fixed text-sm">
        ...
        <tbody>
          {#each groups as entity (entity.entityKey)}
            ...
            {@const modules = [...new Set(entity.moduleIds.map((definitionId) => moduleLabelForDefinition(definitionId)))]}
                        ...
                        >
                          <div class="min-w-0">
                            <div class="flex items-center gap-1.5">
                              <span class={cn('size-1.5 rounded-full shrink-0', severityDot(alert.severity))}></span>
                            ...
                            <div class="mt-0.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
                              <span>{moduleLabelForDefinition(alert.definitionId ?? '')}</span>
